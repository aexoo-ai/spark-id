...
const secure_id_1 = require("./lib/secure-id");
function parseArgs() {
...
function outputIds(ids, format = 'text') {
    switch (format) {
        case 'json':
            if (ids.length === 1) {
...
function main() {
    ...
    // Validation mode
    if (options.validate) {
        const isValid = (0, secure_id_1.isValidId)(options.validate);
        console.log(isValid);
    ...
    if (options.parse) {
        try {
            const parsed = (0, secure_id_1.parseId)(options.parse);
    ...
    for (let i = 0; i < count; i++) {
        ids.push((0, secure_id_1.generateId)(options.prefix));
    }
